...
|---|---|
| `auth.py` | Resolves the `Authorization: Bearer gk-...` header to an `ApiKey` row (and, through it, its `Account`) |
| `ratelimit.py` | Per-account token bucket in Redis |
| `budget.py` | Per-account monthly USD spend cap and alerts |
| `cache_exact.py` | Redis exact-match response cache |
| `cache_semantic.py` | Postgres + `pgvector` embedding-similarity cache |

## Rate limiting

Each **account** gets a per-minute token bucket, `rate_limit_tokens_per_min`
(default 100) refilled at `rate_limit_refill_rate` tokens/second. Rate limiting is
pooled at the account: every key on an account draws from the one shared bucket.
Once the bucket is empty the gateway returns 429 with a `Retry-After` header
...
Budget is pooled at the account: an account with `monthly_budget_usd` set is
blocked once its current calendar-month spend (summed across all of its keys)
reaches that cap; the request fails with a 429 carrying an OpenAI-shaped error
body. Accounts with `monthly_budget_usd` unset are unlimited and skip the check
entirely.

```bash
gatekeep key set-budget my-account 25.00     # cap this account at $25/month
gatekeep key set-budget my-account --unlimited
```

Spend is tracked in a Redis counter per account and period, seeded from
`request_logs` if the counter is missing, and reset implicitly at each month
boundary because the period label (`YYYY-MM`) is part of the counter key. Counters
carry a 40-day TTL.
...
`spend >= account.monthly_budget_usd` regardless of the warning threshold.
...
Both caches are partitioned per account: a cached response is only ever served
back to the account that produced it, so one tenant's completion never leaks to
another.
...
- **Exact cache** (Redis) - identical requests (same model, messages, `max_tokens`,
  and so on) from the same account are served from cache within
  `cache_exact_ttl_seconds`, default 7 days. Keys are namespaced by account id.
- **Semantic cache** (Postgres + `pgvector`) - a request whose embedding is more
  ...
  similarity) to a previously cached request *from the same account* is served
  that cached answer, even with different wording. Observed similarities are
  recorded in `gatekeep_cache_semantic_similarity`.
...
|---|---|---|
| `RATE_LIMIT_TOKENS_PER_MIN` | `100` | Bucket size per account |
