...
/**
 * Forgot-password form: email input, submits to `requestReset()`. Always
 * shows the same neutral confirmation message regardless of whether the
 * email exists, to avoid account enumeration.
 *
 * @param props - See {@link ForgotPasswordPageProps}.
 */
export default function ForgotPasswordPage({ onBackToLogin }: ForgotPasswordPageProps) {
  ...
  /**
   * Submits the forgot-password form: calls `requestReset()` with the
   * current email. Always shows the neutral confirmation afterward, even
   * if the request fails, so the response can't be used to enumerate
  ...
  return (
    <div className="mx-auto mt-16 w-full max-w-sm rounded-lg border border-slate-800 bg-slate-900 p-6">
      <h1 className="mb-4 text-lg font-semibold text-slate-100">Forgot password</h1>
      {submitted ? (
        ...
        <form onSubmit={handleSubmit}>
          <label htmlFor="forgot-email" className="mb-1 block text-sm text-slate-300">
            Email
          </label>
          <input
            id="forgot-email"
