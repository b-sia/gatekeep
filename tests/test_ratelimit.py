...
async def test_check_rate_limit_allows_up_to_capacity():
    ...
    for _ in range(3):
        allowed, remaining = await check_rate_limit(
            redis, key_id=1, capacity=3, refill_rate=0.001, now=1000.0
        )
...
async def test_check_rate_limit_refills_over_time():
    redis = get_redis()
    for _ in range(2):
        await check_rate_limit(redis, key_id=3, capacity=2, refill_rate=1.0, now=1000.0)
    # Bucket is now empty; a request 1s later should get exactly one new token.
    allowed, remaining = await check_rate_limit(
        redis, key_id=3, capacity=2, refill_rate=1.0, now=1001.0
    )
    assert allowed is True
    assert remaining == pytest.approx(0.0, abs=1e-6)
